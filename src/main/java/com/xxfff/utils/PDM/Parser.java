...
public class Parser {
	...
	public static void main(String[] args) {
		try {
			new Parser().pdmParser("/pdm/pdmtest.pdm");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public PDM pdmParser(String pdmFileName) throws Exception {
		...
		Document doc = null;
		File file = new File(ProjectProperty.RESOURCES_PATH + "/" + pdmFileName);
		if(file.exists()){
			doc = reader.read(file);
		}else{
			InputStream is=this.getClass().getResourceAsStream(pdmFileName);
			doc = reader.read(is);
		}
	...
	public ArrayList<PDMColumn> pdmColumnParser(Node node) {
		...
		for (Object o : node.selectNodes("c:Columns/o:Column")) {
			...
			pdmColumn.setDataType(columnNode.selectSingleNode("a:DataType")
					.getText());
			pdmColumn.setType(TypeConvert.getTypeString(pdmColumn.getDataType()));
			pdmColumn
	...
	public ArrayList<PDMReference> pdmReferenceParser(Node node)
		...
		for (Object reference : node.selectNodes("c:References/o:Reference")) {
			...
			pdmReference.setCode(referenceNode.selectSingleNode("a:Code")
					.getText());
			pdmReference.setCardinality(referenceNode.selectSingleNode("a:Cardinality")
					.getText());
			pdmReference.setComment(referenceNode.selectSingleNode("a:Comment")
					.getText());
